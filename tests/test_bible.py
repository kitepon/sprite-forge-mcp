...
def test_three_stages_each_stop_for_correction(tmp_path, monkeypatch):
    ...
    assert "Use this character sheet as the only reference" in first["20"]["inputs"]["prompt"]
    assert "Only one character" in first["20"]["inputs"]["prompt"]
    assert first["21"]["inputs"]["prompt"] == job["panel_requests"][0]["negative"] and first["22"]["inputs"]["width"] == 832
    assert "4" not in first and first["10"]["inputs"]["image"].startswith("sf_sheet_")
    assert "multiple people" in first["21"]["inputs"]["prompt"]
    ...
    assert dress["20"]["inputs"]["prompt"] == dress_req["instruction"] and dress["21"]["inputs"]["prompt"].endswith("frills, boots") and dress["23"]["inputs"]["seed"] == 9
