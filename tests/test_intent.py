...
def test_sheet_keeps_reference_layout():
    ...
    assert "character design sheet" in prompt and "expression sheet" in prompt
    ...
    assert sheet_content(with_expression).endswith("blue jacket, slight smile")
...
def test_unresolved_style_change_is_not_persisted(tmp_path, monkeypatch):
    service, _ = make(tmp_path, monkeypatch)
