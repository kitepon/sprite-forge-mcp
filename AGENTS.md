...
- キャラクター LoRA はまず参考画像から作る。プレビューは同じ seed で、制作への注文なし10枚と注文あり10枚を並べる。形式文は全身だけにし、standing / front view / looking at viewer は入れない。OKにしたプレビューを教材に足し、サンプルと同じ LoRA 学習で更新する。学習が終わったら新しい二組のプレビューを表示する。安定するまで繰り返し、そのあと一枚シートへ進む。NGの選好対でLoRAを上書きする経路は使わない。
