...
- プレビュー採用のあとに一枚シートの合否を置く。一枚シートは、同じ子・同じ衣装で、正面・斜め・横・背面、表情、さまざまなポーズを一枚にまとめた参照シートであり、同じ絵の重複は出さない。プレビューの単体全身と同じ注文にしない。`SINGLE_VIEW_NEGATIVE` は設定画の各パネルだけに使う。合格した一枚を設定画の起点にする。新しい複数パネル生成器は作らない。
- 設定画のパネルは、合格した一枚シートをベースに、Anima と学習済み LoRA とパネルの制御ワードで描く。シート全体を骨格に入れると複数人が写るので、シート上の全身を左から正面・横・斜め・背面として使い、そのパネルの向きの一体を pose にする。合格シートがなければ設定画は始めない。パネルの修正内容はキャラクター台帳へ残し、次の生成でも使う。
