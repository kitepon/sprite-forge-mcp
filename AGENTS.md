...
- キャラクター LoRA はまず参考画像から作る。学習画面の開始・作り直しも参考画像だけでゼロから作る。OKで足したプレビュー教材と、設定画の採用パネル教材は、その操作で外れる。プレビューは同じ seed で、制作への注文なし10枚と注文あり10枚を並べる。形式文は全身と白地。standing / front view / looking at viewer は入れない。プレビューのOK教材はプレビュー画面が足す。設定画の採用パネルは設定画画面が足す。学習が終わったら新しい二組のプレビューを表示する。安定するまで繰り返し、そのあと設定画へ進む。NGの選好対でLoRAを上書きする経路は使わない。
...
- プレビューの生成文には、その回の制作への注文を載せる。サンプル解釈で残った persistent な衣装・体形・顔は、注文で言い直さない限り生成文へ足さない。OKにしたプレビューの学習文はトリガーだけにする。衣装の注文文は生成に使い、教材へは載せない。
- 一枚シートは作らない。プレビューのあと設定画へ進む。設定画はパネルごとに10枚描き、利用者がその中から採用する。採用済みのパネルも、同じ10枚出しから差し替えられる。全パネルを一括生成しない。「設定画を全部作り直す」は空の設定画にして先頭パネルから10枚を出す。1枚採用すると次の未採用パネルの10枚へ進む。
- 設定画のパネルは、学習済み LoRA とそのパネルの指令で Anima が描く。指令には「描いてよいのは1人だけ」を入れる。`SINGLE_VIEW_NEGATIVE` は各パネルに使う。パネルの修正内容はキャラクター台帳へ残し、次の生成でも使う。
- 採用したパネル画像で LoRA を追加学習できる。開始は明示的な操作。プレビューの OK 教材と同じく、学習画面の作り直しでは外れる。
...
- 画像は選択すると追加し、転送中・追加済み・失敗を表示する。希望と説明は自動保存し、学習は「この内容で学習を始める」の一回の操作で、画像の読み取り・教材作成・学習まで連続実行する。作り直しは参考画像だけで LoRA をゼロから作る。未回答の質問がある場合だけ止める。AIの説明と採用理由は学習中・完了後も読める状態で残し、閲覧や承認を毎回の必須操作にしない。
