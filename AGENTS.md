...
- プレビュー採用のあとに一枚シートの合否を置く。一枚シートは向きを並べた参照シートであり、プレビューの単体全身と同じ注文にしない。表情の種類は既定の注文に載せない。利用者が台帳に書いたときだけ足す。`SINGLE_VIEW_NEGATIVE` は設定画の各パネルだけに使う。合格した一枚を設定画の起点にする。新しい複数パネル生成器は作らない。
