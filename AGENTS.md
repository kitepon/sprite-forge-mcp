...
- プレビュー判定の再学習は、NGで選んだ部位のSAMマスク上でOK／NGの選好更新をする。画風だけのNGと部位指定のない対は画面全体で更新する。コメント解釈は、部位指定とユーザーの文で観察範囲を決めてから行い、指定外の全身観察はしない。質問と、部位指定がないときの一般語による部位の補完に使い、理由文を生成文へ足して再学習したと扱わない。
