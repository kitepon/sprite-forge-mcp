...
- プレビュー採用のあとに一枚シートの合否を置く。一枚シートは、同じキャラクターの向きと表情を一枚にまとめた参照シートであり、プレビューの単体全身と同じ注文にしない。`SINGLE_VIEW_NEGATIVE` は設定画の各パネルだけに使う。合格した一枚を設定画の起点にする。新しい複数パネル生成器は作らない。
