...
- キャラクター LoRA はまず参考画像から作る。学習画面の開始・作り直しも参考画像だけでゼロから作る。OKで足したプレビュー教材は、その操作で外れる。プレビューは同じ seed で、制作への注文なし10枚と注文あり10枚を並べる。形式文は全身と白地。standing / front view / looking at viewer は入れない。OKにしたプレビューを教材に足す更新はプレビュー画面だけが行う。学習が終わったら新しい二組のプレビューを表示する。安定するまで繰り返し、そのあと一枚シートへ進む。NGの選好対でLoRAを上書きする経路は使わない。
...
- 設定画のパネルは、学習済み LoRA と合格した一枚シートと、そのパネルの指令で Anima が描く。指令には「描いてよいのは1人だけ」を入れる。シートの画素を切り貼りしてパネルにしない。表情はシートの顔を初期画像にする。衣装が変わるパネルは服の画素を渡さず、シートの骨格だけを取る。合格シートがなければ設定画は始めない。パネルの修正内容はキャラクター台帳へ残し、次の生成でも使う。
