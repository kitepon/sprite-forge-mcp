...
- プレビュー判定の再学習は、NGで選んだ部位のSAMマスク上でOK／NGの選好更新をする。画風だけのNGと部位指定のない対は画面全体で更新する。コメント解釈は、部位指定とユーザーの文で観察範囲を決めてから行い、指定外の全身観察はしない。質問と、部位指定がないときの一般語による部位の補完に使う。判定コメントの理解は他の注文と同じく生成文まで出し、再プレビューと学習はその生成文を読む。日本語コメントの原文を生成文へ貼らず、画風を表す語句も埋め込まない。
...
- 姿（衣装・体形・顔）は LoRA が持つ。プレビューと一枚シートと設定画の文章は、形式用の文と、再学習で一本にした差分だけ。台帳の persistent な衣装・体形条件をプロンプトへ足さない。
