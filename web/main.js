...
import { h, $, icon, button, link, field, picture, empty, notice, action, pageHead, errorState, dateText, confirmAction } from './ui.js';
import { FLOWS, flow, cover, openFlow } from './flows.js';
import { jobs, operations, active, connectionError, subscribe, refreshJobs, startJobUpdates, tickElapsed, terminal, kindLabel, imagePaths, jobView, taskPanel } from './jobs.js';
...
async function home(root) {
  ...
  root.replaceChildren(h('section', { class: 'hero' }, h('div', { class: 'hero-copy' }, h('p', { class: 'eyebrow' }, h('span', { class: 'tiny-dot' }), 'LET’S MAKE SOMETHING YOURS'), h('h1', {}, 'そのキャラの、', h('br'), h('span', {}, '次の一枚を。')), h('p', { class: 'hero-description' }, '好きな画像を集めて、らしさを覚えて。', h('br'), '確かめながら、少しずつ形にしていこう。'), link(['キャラクターを作る', icon('arrow')], '#/flow/sheet', 'button-link hero-cta'), h('p', { class: 'hero-footnote' }, '途中からでも、何度戻っても。')), art),
    ...
    h('section', { class: 'section stack' }, h('div', { class: 'section-heading' }, h('div', {}, h('p', { class: 'eyebrow' }, 'YOUR CHARACTERS'), h('h2', {}, '制作の続きを')), link(['すべて見る', icon('arrow', 17)], '#/library?tab=characters', 'text-link')),
      characters.length ? h('div', { class: 'home-characters' }, characters.slice(0, 4).map(rec => h('article', { class: 'continue-card' }, h('div', { class: 'continue-cover' }, picture(cover(rec), rec.name)), h('div', { class: 'stack' }, h('div', { class: 'section-heading' }, h('h3', {}, rec.name), h('span', { class: `badge ${rec.lora_name ? 'green' : ''}` }, rec.lora_name ? '学習済み' : '準備中')), h('p', { class: 'muted small' }, `${rec.samples.length} 枚の参考画像${rec.bible ? ' · 設定画あり' : ''}`), button(['続きを作る', icon('arrow', 16)], () => openFlow('sheet', rec.name, 'character', rec.bible || rec.approved_sheet ? 5 : rec.pending_sheet ? 4 : rec.lora_name ? 3 : 1), 'quiet'))))) : empty('あなたのキャラクターが、ここに並びます', 'まずは気に入った画像から始めてみましょう。', link([icon('plus'), '最初のキャラクター'], '#/flow/sheet'))),
