...
async function previewStep(target, ctx, styled, cleanup, setReady, next) {
  ...
  target.append(h('p', {}, '制作への注文で望む衣装を出します。OKにした絵を教材に足してLoRAを更新し、安定するまで繰り返してから設定画へ進みます。'), wishes);
  let gallery;
...
async function sheet(target, ctx, styled, cleanup) {
  ...
  await API.bible(name, 1, '', style);
  let rec = await API.character(name);
  const layout = await layoutEditor(target, name, cleanup);
  const existing = h('div', { class: 'stack' }); const edit = h('div', { class: 'stack' });
  ...
  target.append(h('p', {}, 'パネルを選んで10枚出します。その中から採用し、全パネルについて繰り返します。採用済みも、また10枚出して差し替えられます。'),
    grow, existing, edit);
  const refreshEditor = await redraw(edit, name, rec, cleanup, showExisting, style);
...
async function redraw(target, name, rec, cleanup, updated, style = '') {
  ...
  await loadComment();
  // 候補: 同じ指令で seed だけ変えた 10 枚を並べ、選んだ一枚だけ設定画へ入れる。
  ...
  const showCandidates = job => {
    ...
    candidates.replaceChildren(h('h3', {}, `${label} の候補`),
      h('p', { class: 'muted' }, job.adopted ? '採用した一枚を設定画へ入れました。別の候補に変えることもできます。' : '気に入った一枚を選ぶと、このパネルに入ります。選ぶまで設定画は変わりません。'),
  ...
  target.append(h('div', { class: 'stack' }, h('h3', {}, 'パネルを選んで10枚から採用する'), h('p', { class: 'muted' }, '項目を選んで押すと10枚描きます。採用済みのパネルも、もう一度10枚出して差し替えられます。'), picker, selectedTitle,
...
export function flow(root, id) {
  ...
  const hints = { sheet: ['まず、作りたい子を選びましょう。', '画像と説明を一枚ずつ確かめましょう。', 'ここで初めて学習を始めます。', '顔や衣装を見て、先へ進むか決めましょう。', 'パネルごとに10枚から、残す一枚を選びます。'], draw: ['描きたいキャラクターを選びましょう。', '思い浮かべた場面を、言葉にしてみましょう。'], restyle: ['画風を変えたいキャラクターを選びましょう。', '試してみたい画風を選びましょう。', '顔と衣装が保たれているか確かめましょう。', '選んだ画風で、パネルごとに10枚から採用します。'], style: ['この画風に、名前をつけましょう。', '好きな線や色づかいが伝わる画像を。', '画像の描き方を覚えます。', '別の被写体でも、好きな絵になりますか？'], styleonly: ['使いたい画風を選びましょう。', '被写体は自由に。言葉から描いてみましょう。'] };
  ...
  const refreshContext = async supplied => {
    ...
    furthest = !rec ? 0 : id === 'sheet' ? (rec.lora_name ? spec.steps.length - 1 : rec.samples.length ? 2 : 1)
      : id === 'style' ? (rec.lora_name ? spec.steps.length - 1 : rec.samples.length ? 2 : 1)
      : id === 'restyle' ? (rec.lora_name && ctx.style ? spec.steps.length - 1 : 1)
      : spec.steps.length - 1;
