...
export const API = {
  ...
  startPreviewPair: (name, tags, seed = 1, count = 10, style = "", intent_job_id = "") => api(`/api/characters/${encodeURIComponent(name)}/preview-pair?${new URLSearchParams({ tags, seed, count, style, intent_job_id })}`, { method: "POST" }),
  ...
  retryPanel: (name, panel, count = 4, style = "") => api(`/api/panel/retry?${new URLSearchParams({ name, panel, count, style })}`, { method: "POST" }),
