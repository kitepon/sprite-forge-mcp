...
export const intentCaption = job => job?.kind === 'lora_train' && job.status === 'awaiting_confirmation' ? '教材の確認待ち・学習は未開始' : job?.kind === 'intent' ? ({ draft: '原文を保存済み・未解釈', awaiting_confirmation: '解釈案の確認待ち', confirmed: '確認した条件を採用済み' }[job.status] || '') : '';
...
export function jobView(job, { title, startedAt, error, requesting, notStarted, hideImages } = {}) {
  ...
  const view = h('section', { class: `job-view ${failed ? 'failed' : done ? 'completed' : ''}` },
    ...
    !resting && !done && !failed && !notStarted ? h('p', { class: 'muted small' }, job?.kind === 'intent' ? '原文は保存されています。解釈案ができたら、注文を入力した工程で確認できます。' : job?.kind === 'lora_train' ? '学習出力の実測値を表示します。この画面を離れても「制作状況」で確認できます。' : 'できた画像からここに並びます。画面を移動しても「制作状況」で確認できます。') : null,
