...
export function progress(job) {
  ...
  if (['lora_train', 'preview_learning', 'lora_grow'].includes(job?.kind) && job.progress?.total > 0 && job.status !== 'previewing') return { value: job.progress.step, total: job.progress.total, unit: 'step' };
  ...
  if (job?.kind === 'panel_retry_all' && job.total_panels) return { value: job.completed_panels || 0, total: job.total_panels, unit: 'パネル' };
  return null;
}
...
export async function runJob(spec, title, request, existingJob = null) {
  ...
  if (active(operations.get(key))) return;
  const op = { spec, title, baseline: jobs.map(job => job.job_id), startedAt: new Date().toISOString(), requesting: true, job: existingJob, error: '' };
...
export function jobView(job, { title, startedAt, error, requesting, notStarted, hideImages } = {}) {
  ...
  const caption = intentCaption(job) || (done ? 'できました' : failed ? '処理に失敗しました' : notStarted ? '入力を確認してください' : job?.kind === 'preview_learning' && job.status === 'interpreting' ? '判定を読んでいます' : job?.kind === 'preview_learning' && job.status === 'training' ? '判定の内容で LoRA を学習しています' : job?.kind === 'preview_learning' && job.status === 'previewing' ? '学習結果のプレビューを生成しています' : job?.kind === 'lora_grow' && job.status === 'running' ? '教材を足して、学習の準備をしています' : job?.kind === 'lora_grow' && job.status === 'training' ? 'OKの画像を教材に足して学習しています' : job?.kind === 'lora_grow' && job.status === 'previewing' ? '学習後のプレビュー（注文なし／あり）を生成しています' : job?.kind === 'preview_pair' && job.status === 'running' ? '注文なし10枚と注文あり10枚を生成しています' : job?.kind === 'intent' ? '画像と注文を解釈しています' : job?.status === 'queued' ? '準備・GPU の処理待ち' : job ? '最後の報告：処理中' : requesting ? '開始の応答を待っています' : '応答を確認してください');
  ...
  const view = h('section', { class: `job-view ${failed ? 'failed' : done ? 'completed' : ''}` },
    ...
    p ? h('div', { class: 'progress-wrap' }, h('progress', { max: p.total, value: p.value, 'aria-label': '制作の進捗' }), h('span', {}, `${p.value} / ${p.total} ${p.unit}`)) : !resting && !done && !failed && !notStarted ? h('div', { class: 'indeterminate', 'aria-label': '処理待ち。進捗率は未取得' }, h('span')) : null,
    ...
    !resting && !done && !failed && !notStarted ? h('p', { class: 'muted small' }, job?.kind === 'intent' ? job.learning_steps !== undefined ? '読み取りが終わると、教材を準備して学習へ進みます。回答が必要な質問がある場合だけお知らせします。' : '原文は保存されています。解釈案ができたら、注文を入力した工程で確認できます。' : job?.kind === 'lora_train' ? '学習出力の実測値を表示します。この画面を離れても「制作状況」で確認できます。' : 'できた画像からここに並びます。画面を移動しても「制作状況」で確認できます。') : null,
...
export function taskPanel(spec, title, startLabel, request, cleanup, onComplete, options = {}) {
  ...
  const start = h('button', { type: 'button', class: 'primary', onclick: () => runJob(spec, title, request, options.existingJob?.()) }, icon('spark'), startLabel);
